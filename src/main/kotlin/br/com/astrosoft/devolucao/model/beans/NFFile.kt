...
class NFFile(
  val storeno: Int, val pdvno: Int, val xano: Int, val date: LocalDate, var nome: String, var file: ByteArray
            ) {
  ...
  companion object {
    fun new(nota: NotaSaida, fileName: String, bytes: ByteArray): NFFile {
      return when (nota.tipo) {
        "PED" -> NFFile(
          storeno = nota.loja, pdvno = 9999, xano = nota.pedido, date = LocalDate.now(), nome = fileName, file = bytes
                       )
        else  -> NFFile(
          storeno = nota.loja,
          pdvno = nota.pdv,
