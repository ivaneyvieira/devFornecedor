...
class DlgEditRmk {
    fun editRmk(nota: NotaSaida, save: (NotaSaida) -> Unit) {
        val form = SubWindowForm(
                "PROCESSO INTERNO: ${nota.nota}|DEV FORNECEDOR: ${nota.fornecedor}",
                toolBar = { window ->
                    button("Salva") {
                        icon = CHECK.create()
                        onLeftClick {
...
class DlgSelecionaEmail(val viewModel: TabDevolucaoViewModelAbstract) {
    fun selecionaEmail(nota: NotaSaida, emails: List<EmailDB>) {
        val form =
            SubWindowForm("PROCESSO INTERNO: ${nota.nota}|DEV FORNECEDOR: ${nota.fornecedor}") {
    ...
    private fun createGridNotas(listNotas: List<NotaSaida>, serie: String): Grid<NotaSaida> {
        ...
        return gridDetail.apply {
            ...
            if (serie == "PED") sort(
                    ...
                    listOf(
                            GridSortOrder(
                                    getColumnBy(NotaSaida::dataNota),
                                    SortDirection.ASCENDING
                                         )
                          )
                     )
