...
SELECT iprd2.storeno                                                            AS lj,
       ...
       inv2.invse                                                               AS serie,
       IF(baseIcms = 0, 0.00, IF(MID(iprd2.cstIcms, 2, 3) = '20',
                                 ROUND(iprd2.baseIcms * 100.00 / (iprd2.fob * (iprd2.qtty / 1000)), 4),
                                 NULL))                                         AS icmsd,
       ...
       IF(inv2.weight = 0, NULL, inv2.weight)                                   AS pesoBrutoTotal,
       @PESO_BRUTO * (iprd2.qtty / 1000)                                        AS pesoBrutoPrd,
       @FRETE := ROUND((inv2.freight / 100) / (inv2.weight), 4)                 AS freteKg,
       @FRETE_UN := ROUND(prd.weight_g * @FRETE, 4)                             AS freteUnit,
       ROUND(@FRETE_UN * 100 * 10000 / iprd2.fob4, 2)                           AS fretePerNf,
       prp.freight / 100                                                        AS fretePerPrc,
       iprd2.qtty / 1000                                                        AS quant,
       S.estoque                                                                AS estoque,
       mesesValidade                                                            AS mesesValidade,
       iprd2.cstIcms                                                            AS cstIcms,
       iprd2.cfop                                                               AS cfop,
       ROUND(iprd2.dfob * iprd2.qtty / 1000, 2)                                 AS valor,
       ROUND(iprd2.discount / 100, 2)                                           AS vlDesconto,
       ROUND(iprd2.dfob * iprd2.qtty / 1000, 2) -
       ROUND(iprd2.discount / 100, 2)                                           AS vlLiquido,
       ROUND(ROUND(iprd2.dfob * iprd2.qtty / 1000, 2) * iprd2.frete / 10000, 2) AS vlFrete,
       ROUND(ROUND(iprd2.dfob * iprd2.qtty / 1000, 2) * iprd2.despesas / 10000,
             2)                                                                 AS vlDespesas,
